...
test.describe("CounterCard", () => {
  ...
  test("Default story renders correctly", async ({ page }) => {
    ...
    await expect(page.getByText("Total de Vagas")).toBeVisible({ timeout: 10000 });
    await expect(page.getByText("78")).toBeVisible();
    await expect(page.getByText("/100")).toBeVisible();
    await expect(page.getByText("Acompanhe o número de vagas disponíveis")).toBeVisible();
  });

  test("Percentage story renders correctly", async ({
    page,
  }) => {
    await page.goto("/iframe.html?id=components-countercard--percentage");
    await page.waitForLoadState('networkidle');
    
    await expect(page.getByText("Vagas Preenchidas")).toBeVisible({ timeout: 10000 });
    await expect(page.getByText("78")).toBeVisible();
    await expect(page.getByText("/100")).toBeVisible();
  ...
  test("HighUsage story renders correctly", async ({
    page,
  }) => {
  ...
  test("WithoutMaxCount story renders correctly without max count", async ({
    ...
    await page.goto("/iframe.html?id=components-countercard--without-max-count");
    await page.waitForLoadState('networkidle');
    
    await expect(page.getByText("Visitantes Online")).toBeVisible({ timeout: 10000 });
